...
TEST_CASE("Verifying the constructors", "[matrix]") {
    ...
    SECTION("default constructor") {
        ...
        REQUIRE(mat1.size() == make_pair(s1, s2));
    }
    ...
    SECTION("1d vector constructor") {
        ...
        for (size_t i = 0; i != mat3.size().first; ++i)
            for (size_t j = 0; j != mat3.size().second; ++j)
                REQUIRE(mat3(i, j) == v3_1[i * mat3.size().second + j]);
    ...
    SECTION("2d vector constructor") {
        ...
        matrix<int> m5(v5);
        for (size_t i = 0; i != m5.size().first; i++)
            for (size_t j = 0; j != m5.size().second; j++)
